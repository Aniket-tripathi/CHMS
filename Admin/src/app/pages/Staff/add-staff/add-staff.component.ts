...
export class AddStaffComponent {
  ...
  ngOnInit(): void {
    ...
    if (clinicId) {
      this.fetchClinicsbyId(clinicId);
      this.staffForm.controls['clinicId'].setValue(clinicId); // Set clinicId in the form
    } else {
      this.fetchClinics();
    }
  ...
  fetchClinicsbyId(clinicId: string): void {
    this.clinicService.getClinicById(clinicId).subscribe({
      next: (clinic) => {
        this.clinicData = [clinic]; // Assign only the single clinic
      },
      error: (err) => {
  ...
  fetchdepartment(): void {
    this.ApiService.getd().subscribe((response: any) => {
